...
gulp.task('lint-all-html', gulp.series(done => {
  ...
  const proc = childProcess.spawn('node', ['./start-test-server.js'], { PORT: port });
...
gulp.task('browserify', gulp.series(() =>
  browserify('app/assets/js/main.js')
    ...
    .pipe(plumber())
    .pipe(source('main.js'))
    .pipe(streamify(babel({ presets: ['@babel/preset-env'] }))) // babel doesn't support streaming
...
gulp.task('check-content', () => {
  ...
  return checkForDeadLinks(activitiesWithBody).then(result => {
    if (result.brokenLinksCount > 0) {
      log(c.red('Found broken links:'));
      log(JSON.stringify(result, null, '  '));
      process.exit(1);
    } else {
      log('All links checked and are OK.');
    }
  });
});
